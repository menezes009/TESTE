// ios-dashboard.js – local panel for iOS
...
function getUsed() { return new Set(JSON.parse(localStorage.getItem(`used_${EVENT_NAME}`) || '[]')); }
function getCheckins() { return JSON.parse(localStorage.getItem(`checkins_${EVENT_NAME}`) || '[]'); }

async function refresh() {
  const codes = await getCodes();
  const used = getUsed();
  const all = Array.isArray(codes) ? codes.map(r => String(r.codigo||'').trim()).filter(Boolean) : Object.keys(codes);
  ...
  const rows = arr
    ...
    .map(c => {
      ...
      return `<tr>
        ...
        <td>${escapeHtml(c.deviceId||'ios')}</td>
      </tr>`;
    }).join('');
