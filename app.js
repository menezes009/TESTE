// app.js – pacote único (iOS-friendly)
// - html5-qrcode (camera + file)
// - Duplicate prevention + log via localStorage
// - Supports data.json as MAP or ARRAY

// Storage keys (sem nome de evento para evitar inconsistências)
...
let codesMap = {}; // {CODE: {name, presenca?}}
...
async function listCameras(){
  ...
  cams.forEach(c => {
    ...
    o.value = c.id; o.textContent = c.label || c.id; elSel.appendChild(o);
  });
  if (cams[0]) {
    // preferir traseira
    const back = cams.find(c => /back|rear|traseira/i.test(c.label||''));
    currentCameraId = (back && back.id) || cams[0].id;
    elSel.value = currentCameraId;
  }
}
...
function handleDecodedText(text){
  ...
  const now = Date.now();
  if (code === last && now - lastTime < DEBOUNCE) return;
...
elBtnImg.addEventListener('click', async () => {
  try{
    ...
    if (!f){ setStatus('Selecione JPG/PNG', 'warn'); return; }
