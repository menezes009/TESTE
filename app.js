// app.js – Fresh Start build
// Namespace
const KEY_PREFIX = 'qr_v4';
const USED_KEY = `${KEY_PREFIX}_used`;
const LOG_KEY  = `${KEY_PREFIX}_checkins`;

// FRESH START: clear any old data on page load so the first scan is always free
(function freshStart(){
  try {
    const toDelete = [];
    for (let i=0; i<localStorage.length; i++) {
      const k = localStorage.key(i);
      if (/qr_v\d+_|(^qr_(used|checkins)$)|used|checkin/i.test(k)) toDelete.push(k);
    }
