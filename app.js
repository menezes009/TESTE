// app.js – Hard Reset build
// New namespace to avoid old data conflicts
const KEY_PREFIX = 'qr_v3';
...
function hardReset(){
  ...
  // Also try to remove common legacy keys
  const candidates = Object.keys(localStorage).filter(k => /used|checkin/i.test(k));
...
// Optional: allow reset via URL ?reset=1
if (location.search.includes('reset=1')) hardReset();
