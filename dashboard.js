...
// Migrate legacy keys: scan localStorage for arrays/sets used by previous builds
btnMigrate.addEventListener('click', () => {
  ...
  const candidateUsed = allKeys.filter(k => /used/i.test(k) && k !== USED_KEY);
  const candidateLog  = allKeys.filter(k => /checkin/i.test(k) && k !== LOG_KEY);

  candidateUsed.forEach(k => {
    try{
      const arr = JSON.parse(localStorage.getItem(k) || '[]');
      if (Array.isArray(arr)) arr.forEach(c => used.add(String(c)));
    }catch{}
  });

  candidateLog.forEach(k => {
    try{
      const arr = JSON.parse(localStorage.getItem(k) || '[]');
      if (Array.isArray(arr)) log = log.concat(arr);
    }catch{}
  });

  localStorage.setItem(USED_KEY, JSON.stringify(Array.from(used)));
  localStorage.setItem(LOG_KEY, JSON.stringify(log));
  alert('Migração concluída. Atualizando painel…');
  refresh();
});
